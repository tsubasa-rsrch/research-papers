...
**Version**: v0.13 (fifteen-condition ablation; twelve mechanistic reversals; state-independence toxicity principle from v2.8 / v2.10p / v2.11p; Echo peer review corrections)
**Date**: 2026-04-08
...
The three independent attempts to add developmental-analog mechanisms on top of the v2.7c dynamic master controller produced the following ablation table row:

| condition | implementation form | F1 (v2.7c = 0.3444) | Δ F1 |
|---|---|---|---|
| v2.8 | discrete periodic event | 0.3339 | **−1.0 pp** |
| v2.10p | continuous static scalar | 0.2868 | **−5.8 pp** |
| v2.11p | per-connection static bias | 0.2990 | **−4.5 pp** |

**All three regress.** The implementations differ in temporal structure (discrete vs continuous), functional form (scalar vs per-connection), and bias shape (nominally symmetric vs asymmetric), yet every one of them reduces Overall F1 by a measurable amount. The mechanism is the same in all three cases: the v2.7c damping controller computes `damp = v_baseline / (v_baseline + v_now)`, a state-relative expression that derives its ability to modulate STDP from the fact that `v_now` is a trustworthy signal of the network's learning dynamics. Any state-independent addition that *also* perturbs the weight dynamics injects noise into `v_now` without carrying real information about learning: a sleep pass moves weights for a reason the controller cannot distinguish from runaway (v2.8), a hormone multiplier on STDP amplitudes silently reduces the amplitude of the signal the controller is measuring (v2.10p), and a per-connection static bias breaks the statistical assumption that connections are interchangeable at the population level (v2.11p).
...
We therefore state the design principle cautiously:

> **Within the class of state-independent additions to a state-dependent controller, every addition tested regresses. This motivates a design commitment for Hux Phase 3+: any mechanism added on top of a dynamic controller must itself be state-dependent. The three cases reported here are the empirical ground; universality beyond this class remains to be tested.**
...
These three rules are not reforms of the v2.7c master controller; they are constraints on *any* future mechanism that would be added *to* the master controller. Together they define a **Hux design constitution** for Phase 3+ that any subsequent version must satisfy before being run as a candidate rescue. The v3.1 gradual-sleep implementation, the v3.2 bidirectional HPT-axis implementation, and the v2.9b symmetric-patterning implementation are the first three concrete targets that this constitution will govern.
