...
**Baseline conditions.** Two gate-only baselines were measured under different solver options. The hippocampal experiments were conducted first using the default solver setting (save_everystep=true, baseline 81.7%, N=30). After discovering a baseline shift with saveat=[trial_dur] (76.0%, N=30), all subsequent amygdala experiments used the saveat setting, and a new matched baseline was collected. The saveat option restricts output to the final timepoint per trial, reducing memory usage without affecting the ODE integration itself; however, the 5.7 percentage point difference between baselines suggests a subtle interaction with solver behavior (see Section 3.5 for discussion of solver-related effects). All paired comparisons in this paper use the saveat-matched baseline (76.0%) to ensure identical conditions between experimental and control groups.
...
Adding a static hippocampus (DG-CA3-CA1, 15 HHNeuronExci, bidirectional cortical connections) to the gate circuit produced a small improvement under saveat-matched conditions: gate+HPC mean 80.1% (SD=12.6) vs gate-only mean 76.0% (SD=8.0) (N=30, paired t=3.00, p=0.006, d=0.55, HPC wins 22/30). However, the hippocampus substantially increased variance (SD ratio 12.6/8.0=1.57), indicating seed-dependent effects: some seeds improved markedly while others degraded, producing an unstable net benefit. Under a different solver condition (save_everystep=true, original baseline 81.7%), the same hippocampal addition yielded 81.6% (SD=7.8), a null result (p=0.98, d=0.01), suggesting that the hippocampal effect is condition-sensitive and should be interpreted cautiously as an exploratory finding. The variance increase was consistent across both conditions. This pattern motivates the search for what additional mechanisms (Hebbian plasticity, entorhinal cortex buffering, cholinergic modulation) are required for stable hippocampal function.

### 3.2 Amygdala to Cortex: Destruction

Adding 10 amygdala neurons (HHNeuronExci) with projections to both association cortex (AC) and thalamic gate reduced accuracy to 49.6% (SD=1.3, N=30), at chance level for the two-category task (paired t=-17.51, p<10^-8, d=-3.20, gate-only wins 30/30). All 30 seeds fell within 47.3-52.1%, indicating complete abolition of learning. A sham control with amygdala projecting to AC only (without gate connection) produced identical results (49.8%, SD=1.6, N=30, paired t=-17.86, d=-3.26), confirming that the cortical projection itself, not the presence or absence of gate input, causes the disruption.
...
Paired t-test: t=17.93, p<10^-8. Cohen's d=3.27. 95% CI: [+11.0, +13.6] percentage points. Amygdala-gate wins 30/30 seeds.
...
| +Hippocampus (static) | 80.1% (SD=12.6) | +4.1pp | 0.006 | 0.55 | exploratory* |
...
*Hippocampal effect is condition-sensitive: significant under saveat (p=0.006) but null under save_everystep=true (p=0.98). Designated exploratory due to condition-sensitivity. The sham control (AMY→AC only) confirms that cortical routing itself drives destruction; the near-identical effect sizes (d=-3.20 vs d=-3.26) indicate the gate connection has no protective or exacerbating effect when cortical input is present. Preliminary results from 190-neuron full-brain models (v1: 55.9%, v2: 51.4%, v3: 50.3%, all N=1 seed 42) are available in the supplementary materials.
...
- The amygdala-gate comparison and its two sham controls were designated as confirmatory; hippocampus and excitatory overdrive are exploratory and reported without multiple comparison correction.
